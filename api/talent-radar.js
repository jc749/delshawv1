...
async function fetchSloanProfile() {
  try {
    ...
    return (data.results || [])
      ...
      .filter(Boolean)
      .join('\n')
      .slice(0, 2000); // Cap profile to avoid token bloat
  } catch {
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      // Cap at 40 articles + 10 episodes, truncate summaries to stay under rate limit
      const topArticles = articles.slice(0, 40);
      const topEpisodes = episodes.slice(0, 10);
      const newsContext = [
        ...topArticles.map(a => `[ARTICLE] "${a.title}" (${a.source})${a.summary ? ` — ${a.summary.slice(0, 120)}` : ''}`),
        ...topEpisodes.map(e => `[PODCAST] ${e.podcast}: "${e.episode}"${e.summary ? ` — ${e.summary.slice(0, 120)}` : ''}`),
      ].join('\n');
