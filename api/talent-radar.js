  ...
  const res = await fetch('https://api.notion.com/v1/pages', {
    ...
    body: JSON.stringify({
      ...
      properties: {
        ...
        'Source URL': prospect.sourceUrl ? { url: prospect.sourceUrl } : { url: null },
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      const articleContext = articles.map(a =>
        `[ARTICLE] "${a.title}" (${a.source}) [URL: ${a.url || 'none'}]\n${a.body || a.summary}`
      ).join('\n\n---\n\n');
...
Return 8-12 prospects as a JSON array. Each object:
{
  ...
  "matchScore": 6-10,
  "link": "talent's social profile URL if mentioned in content, else null",
  "sourceUrl": "exact URL of the source article from the [URL: ...] tag, else null"
}
