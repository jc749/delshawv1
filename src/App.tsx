...
import {
  ...
  RefreshCw, Star, Filter, X, ChevronDown, Eye, Check
} from 'lucide-react';
import {
...
const InsightsView = ({ client, articles, episodes }: any) => {
  ...
  const fetch = async () => {
    ...
    try {
      const data = await postApi<{ result: string }>('analyze', { type: 'negotiation', client, articles, episodes });
      setInsights(ensureStr(data.result));
    } catch { setError(true); }
    finally { setLoading(false); }
  };

  useEffect(() => { fetch(); }, [client.name]);
    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm h-full">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-slate-900">Negotiation Leverage</h3>
        </div>
        
      </div>
      {loading ? <LoadingSkeleton /> : error ? (
        <div className="p-4 bg-rose-50 rounded-xl border border-rose-100 text-xs text-rose-600">
          Failed to load. <button onClick={fetch} className="underline font-bold">Retry</button>
        </div>
      ) : <div className="prose prose-sm max-w-none text-slate-600"><Markdown>{insights}</Markdown></div>}
    </div>
  );
};
...
const StrategyView = ({ client, articles, episodes }: any) => {
  ...
  const load = async () => {
    setLoading(true); setError(false);
    try {
      const data = await postApi<{ result: string }>('analyze', { type: 'strategy', client, articles, episodes });
      setStrategies(ensureStr(data.result));
    } catch { setError(true); }
    finally { setLoading(false); }
  };
    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm h-full">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-slate-900">Monetization Strategy</h3>
        </div>
      </div>
      ...
      ) : <div className="prose prose-sm max-w-none text-slate-600"><Markdown>{strategies}</Markdown></div>}
    </div>
  );
};

// ─── Sloan's Desk ─────────────────────────────────────────────────────────────
const SloanDesk = ({ articles, episodes }: { articles: any[]; episodes: any[] }) => {
  ...
  useEffect(() => { if (articles.length > 0) load(); }, [articles.length]);
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <button className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-[10px] font-bold text-slate-600 hover:bg-blue-50 hover:text-blue-600 transition-all">
                    <Linkedin className="w-3 h-3" /> LinkedIn Post
                  </button>
                  <button className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-[10px] font-bold text-slate-600 hover:bg-sky-50 hover:text-sky-600 transition-all">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {(data.pitches || []).map((p: any, i: number) => (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 + i * 0.1 }}
              ...
              <button className="w-full flex items-center justify-center gap-2 py-2.5 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 transition-all">
...
const TalentRadar = ({ articles, episodes, clients }: { articles: any[]; episodes: any[]; clients: any[] }) => {
  ...
  const runAI = async () => {
    ...
    try {
      const data = await postApi<{ prospects: any[]; newCount: number }>('talent-radar', { articles, episodes, clients });
      setProspects(data.prospects || []);
      if (data.newCount === 0) setError('No new prospects found — all extracted names already exist in your Talent Radar.');
    } catch (err: any) {
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">AI-surfaced prospects from your Notion content + client lookalike data</p>
        </div>
        <div className="flex gap-3">
          ...
          <button onClick={runAI} disabled={running || articles.length === 0}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sorted.map((prospect, i) => (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              key={prospect.id || i} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all group">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="font-bold text-slate-900 text-base">{prospect.name}</h3>
                  {prospect.followersReach && <p className="text-xs text-slate-400 mt-0.5">{prospect.followersReach}</p>}
                </div>
                {prospect.matchScore && (
                  <div className={cn("flex items-center gap-1 px-2.5 py-1 rounded-full text-sm font-bold",
                    prospect.matchScore >= 8 ? "bg-emerald-50 text-emerald-600" :
                    prospect.matchScore >= 6 ? "bg-amber-50 text-amber-600" : "bg-slate-50 text-slate-500")}>
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                <SourceBadge source={prospect.source || 'Manual'} />
                {(prospect.platform || []).map((p: string) => (
                  <span key={p} className="px-2 py-0.5 bg-slate-50 border border-slate-200 rounded-full text-[10px] font-bold text-slate-500 uppercase">{p}</span>
                ))}
              </div>

              {prospect.whyFit && (
                <div className="p-3 bg-indigo-50/60 rounded-xl mb-3">
                  <p className="text-[10px] font-bold text-indigo-600 uppercase tracking-wider mb-1">Why They're a Fit</p>
                  <p className="text-xs text-slate-600 leading-relaxed">{prospect.whyFit}</p>
                </div>
              )}

              {prospect.upsideNotes && (
                <div className="mb-3">
                  <p className="text-[10px] font-bold text-amber-600 uppercase tracking-wider mb-1">Upside</p>
                  <p className="text-xs text-slate-500 leading-relaxed">{prospect.upsideNotes}</p>
                </div>
              )}

              {prospect.sourceArticle && (
                <p className="text-[10px] text-slate-400 mb-3 italic">Source: {prospect.sourceArticle}</p>
              )}

              <div className="flex gap-2 pt-2 border-t border-slate-100">
                {prospect.link ? (
                  <a href={prospect.link} target="_blank" rel="noopener noreferrer"
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-indigo-600 text-white rounded-xl text-[10px] font-bold hover:bg-indigo-700 transition-all">
                    <ExternalLink className="w-3 h-3" /> View Profile
                  </a>
                ) : (
                  <div className="flex-1" />
                )}
                <select className="py-2 px-2 bg-slate-50 border border-slate-200 rounded-xl text-[10px] font-bold text-slate-600 focus:outline-none"
          ...
          <AnimatePresence mode="wait">
            {view === 'sloan' && (
              <motion.div key="sloan" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <SloanDesk articles={articles} episodes={episodes} />
              </motion.div>
            )}
            {view === 'radar' && (
              <motion.div key="radar" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <TalentRadar articles={articles} episodes={episodes} clients={notionClients} />
              </motion.div>
            )}
            {view === 'client' && client && (
