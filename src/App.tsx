...
const PitchEmailModal = ({ pitch, onClose }: { pitch: { hook: string; target: string }; onClose: () => void }) => {
  ...
  const subject = `Story Pitch: ${pitch.target}`;
  const body = `Hi,

I wanted to share a pitch angle that feels timely for ${pitch.target}:

${pitch.hook}

Happy to connect further and provide additional context, talent access, or background on the story.

Best,
Sloan Whiteside-Munteanu
Del Shaw Mackintosh & Kaiserman`;

  const fullEmail = `To: [PR Contact]
Subject: ${subject}
