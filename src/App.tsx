...
const TalentRadar = ({ articles, episodes, clients, cache, onCache }: any) => {
  ...
  return (
    <div>
      ...
      {/* Hero Header */}
      <div className="relative bg-gradient-to-br from-indigo-600 via-indigo-700 to-purple-800 rounded-3xl p-8 mb-8 overflow-hidden">
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-indigo-200 text-sm font-semibold uppercase tracking-wider">Talent Intelligence</span>
            </div>
            <h1 className="text-4xl font-black text-white mb-2">Talent Radar</h1>
            <p className="text-indigo-200 text-sm max-w-md">AI scans your Notion articles and podcast transcripts to surface emerging talent that fits Sloan's ICP — before everyone else finds them.</p>
          </div>
          <div className="flex flex-col gap-3">
            <button onClick={runAI} disabled={running}
              className={cn("flex items-center justify-center gap-2 px-6 py-3 rounded-xl text-sm font-bold text-indigo-700 bg-white transition-all shadow-lg",
                running ? "opacity-70" : "hover:bg-indigo-50 shadow-white/20")}>
              <RefreshCw className={cn("w-4 h-4", running && "animate-spin")} />
              {running ? 'Scanning...' : 'Run AI Scan'}
            </button>
            <button onClick={() => setShowAdd(true)}
              className="flex items-center justify-center gap-2 px-6 py-3 bg-white/10 border border-white/20 rounded-xl text-sm font-bold text-white hover:bg-white/20 transition-all">
